...
# as using pre-calculated columns results in incorrect marginal means
...
if (experiment == 0) {
  ...
  # model fails to converge with zero correlation on items
  # attempting a refit with zero correlation to subjects
  train_max_model <- as.formula(paste(
    dv_transformed,
    "~ 
